...
TEST(ImageAOSTests, Resize16_2to4_Test) {
  ...
  const int result = system("diff -q ../../test-data/big16-4.ppm big16-4-output.ppm");
  EXPECT_EQ(result, 0);
}
...
TEST(ImageAOSTests, Resize16_4to2_Test) {
  ...
  //diff command should have 0 exit code
  // NOLINTNEXTLINE(*-env33-c)
  const int result = system("diff -q ../../test-data/big16-2.ppm big16-2-output.ppm");
  EXPECT_EQ(result, 0);
}
...
TEST(ImageAOSTests, Resize8_2to4_Test) {
  ...
  //diff command should have 0 exit code
  // NOLINTNEXTLINE(*-env33-c)
  const int result = system("diff -q ../../test-data/small8-4.ppm small8-4-output.ppm");
  EXPECT_EQ(result, 0);
}
...
TEST(ImageAOSTests, Resize8_4to2_Test) {
  ...
  //diff command should have 0 exit code
  // NOLINTNEXTLINE(*-env33-c)
  const int result = system("diff -q ../../test-data/small8-2.ppm small8-2-output.ppm");
  EXPECT_EQ(result, 0);
}
