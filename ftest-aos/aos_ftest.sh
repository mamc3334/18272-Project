...
# Define paths
BUILD_DIR="../cmake-build-debug/imtool-aos"      # Replace with the path to your CMake build directory
SOURCE_DIR="../"    # Replace with the path to your CMake project source directory
EXECUTABLE="./imtool-aos"            # Name of the executable after building
...
if [ -x "$EXECUTABLE" ]; then
    ...
    # Run executable with input parameters and record exit code
    "$EXECUTABLE" "$@"
    EXEC_STATUS=$?
    # Stop time
    END_TIME=$(date +%s.%3N)
